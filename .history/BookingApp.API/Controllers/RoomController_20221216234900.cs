...
namespace BookingApp.BookingApp.API.Controllers
{
    ...
    public class RoomController : ControllerBase
    {
        ...
        [HttpPut("update-room")]
        public async Task<IActionResult> UpdateRoom([FromBody] RoomDto roomDto, CancellationToken cancellationToken){

            Room roomType = _roomTypeRepository.RoomTypes.FirstOrDefault(r =>r.Id == request.Id);
            return Ok( new Response<ResponseDefault>()
            {
                State = true,
                Message = ErrorCode.Success,
